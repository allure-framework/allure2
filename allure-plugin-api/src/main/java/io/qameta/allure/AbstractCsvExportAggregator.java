 ...
 * @since 2.0
 */
public abstract class AbstractCsvExportAggregator<T> implements Aggregator {

    private final String fileName;

    private final Class<T> type;

    public AbstractCsvExportAggregator(final String fileName, final Class<T> type) {
    ...
    public void aggregate(final ReportContext context,
        ...
        try (Writer writer = Files.newBufferedWriter(csv)) {
            ...
            try {
                beanWriter.write(getData(testResultService));
            } catch (Exception e) {
    ...
    protected abstract List<T> getData(final TestResultService testResultService);
