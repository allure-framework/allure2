...
const TreeLeafRow: React.SFC<{ node: AllureTreeLeaf }> = ({ node }) => (
  <>
    ...
    <div className={b("name")}>{node.name}</div>
...
const TreeGroupRow: React.SFC<{ node: AllureTreeGroup; expanded: boolean }> = ({
  node,
  expanded,
}) => (
  <>
...
class TreeGroupNode extends React.Component<TreeGroupNodeProps, TreeGroupNodeState> {
  ...
  render(): React.ReactNode {
    ...
    return (
      <div className={b("node")}>
        <div
          className={b("row", { active: selectedGroupId === node.uid })}
...
const TreeChildren: React.SFC<{
  ...
  <>
    {groups
      ...
      : null}
    {leafs ? leafs.map(child => <TreeLeafNode key={child.id} node={child} route={route} />) : null}
  </>
);
