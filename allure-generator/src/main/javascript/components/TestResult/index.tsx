...
const tabs = [
  ...
  },
  {
    name: "Attachments",
    href: "/attachments",
    render: (testResult: AllureTestResult) => (
      <TestResultAttachments testResultId={testResult.id} />
    ),
  },
];
...
export default class TestResult extends React.Component<TestResultProps, TestResultState> {
  ...
  render() {
    ...
    return (
      <Pane>
        <PaneHeader>
          <PaneSubtitle ellipsis={true}>{testResult.fullName}</PaneSubtitle>
          <PaneTitle>{testResult.name}</PaneTitle>
